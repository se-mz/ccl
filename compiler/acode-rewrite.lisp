...
;;;
;;; Copyright 2007-2010 Clozure Associates
;;;
...
(defun acode-maybe-punt-var (var initform)
  (rewrite-acode-form initform (if *acode-rewrite-trust-declarations*
                                   (or (var-declared-type var) t)
                                   t))
  (let* ((bits (nx-var-bits var)))
    (declare (fixnum bits))
    (cond ((logbitp $vbitspecial bits) nil)
          ((logbitp $vbitpunted bits) t)
          (t (nx1-punt-var var initform)
             (setq bits (nx-var-bits var))
             (when (logbitp $vbitpuntable bits)
    ...
    (or (acode-constant-fold-numeric-binop w form1 form2 '+)
        (let* ((target-fixnum-type *nx-target-fixnum-type*)               
               ...
               (newop (cond ((and (subtypep t1 'double-float)
                            ...
                            ((and (target-arch-case ((:x8664 :x8632 :arm) t))
...
(def-acode-rewrite acode-rewrite-ash ash asserted-type (&whole w num amt)
  (or (acode-constant-fold-numeric-binop w num amt 'ash)
      (let* ((maxbits (target-word-size-case
        ...
        (cond ((eql camt 0)
               (update-acode w (%nx1-operator require-integer) (list num)))
              ((and (typep camt fixnum-type)
                    (< camt 0))
               (if (acode-form-typep num fixnum-type trust-decls)
                 (update-acode w (%nx1-operator %iasr)
                               (list (make-acode (%nx1-operator fixnum) (- camt)) num))
                 (if (acode-form-typep num natural-type trust-decls)
                   (if (< (- camt) (arch::target-nbits-in-word
                                    (backend-target-arch *target-backend*)))
                       (update-acode w (%nx1-operator natural-shift-right)
                                     (list num (make-acode (%nx1-operator fixnum) (- camt))))
                       (update-acode w (%nx1-operator progn)
                                     (list (list (make-acode (%nx1-operator require-integer) num)
              ...
              ((and (typep camt 'fixnum)
               ...
               (update-acode w (%nx1-operator %ilsl) (list amt num)))
              ((and (typep camt 'fixnum)
                    (< 0 camt natural-bits)
               ...
               (update-acode w (%nx1-operator natural-shift-left) (list num amt)))
              ((typep cnum 'fixnum)
